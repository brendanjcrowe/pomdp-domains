"""Multimodal Search -- moved verbatim from ``set_transformer/set_transformer/rl/envs/multimodal_search.py``
on 2026-09-13 (plan section 9 of the parent repo's ``refactor_plans.md``) so the env has a
``pdomains-*`` id and can run on the shared RL harness. The particle filter stays in
``set_transformer/rl/particle_filters/multimodal_search.py``. One addition: :func:`make_env`, the
registration's entry point, which builds :class:`MultimodalSearchConfig` from keyword arguments.
...
def make_env(**config_kwargs) -> "MultimodalSearchEnv":
    """Entry point of ``pdomains-multimodal-search-v0``: ``gym.make`` hands registration and
    call kwargs to the entry point, and the class takes a config object, so build it here."""
    return MultimodalSearchEnv(MultimodalSearchConfig(**config_kwargs))
